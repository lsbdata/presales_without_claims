...
########DRG summary table########
drg_agg = data_eps.groupby('DRGs').agg({'estimated_incidence': 'sum', 'estimated_unit_costs': 'mean',  'estimated_carrum_price': 'mean'}).reset_index()
...
annual_projections = pd.DataFrame({"Achievable Savings": [one_year_net_savings, one_year_net_savings,one_year_net_savings,one_year_net_savings,one_year_net_savings], "Eligibile EE": [employee_pop_size,employee_pop_size,employee_pop_size,employee_pop_size,employee_pop_size], "Utilization": [0.20, 0.25, 0.30, 0.40, 0.50], "Incidence Growth": [0, 0.20, 0.20, 0.20, 0.20], "Compounding Growth Constant": [1.00, 1.20, 1.44, 1.73, 2.07], "Carrum Subscription": [1.00, 1.50, 2.00, 2.00, 2.00]})
